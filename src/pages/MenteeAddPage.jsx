...
function MenteeAddPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const data = { username, address, homeaddress, email, phone, emergencyname, emergencyphone, majortype, date, classBE, classFE, classQA };
    console.log(data);
  };
                ...
                <form onSubmit={handleSubmit} className="mb-0 my-2 mx-3 " action="#" method="POST ">
                  ...
                  <div className="bg-white w-full py-2 flex items-center justify-between rounded text-sm font-semibold">
                    ...
                    <select className=" w-3/4 h-10 text-sm rounded-lg md:text-md focus:bg-[#F8F8F8] opacity-50">
                      <option className="p-2 text-sm hover:bg-sky-500 hover:text-white" value={classFE} onChange={(e) => setFE(e.target.value)}>
                        Frontend Engineering
                      </option>
                      <option className="p-2 text-sm hover:bg-sky-500 hover:text-white" value={classBE} onChange={(e) => setBE(e.target.value)}>
                        Backend Engineering
                      </option>
                      <option className="p-2 text-sm hover:bg-sky-500 hover:text-white" value={classQA} onChange={(e) => setQA(e.target.value)}>
                        Quality Engineering
                      </option>
                    </select>
                  </div>
